...
DOC_LICENSE = "영업등록증"  # 영업등록증/영업허가증/영업신고증
DOC_BIZ_REG = "사업자등록증"
DOC_FACTORY_REG = "공장등록증명서"
DOC_AD_REVIEW = "광고심의필증"
DOC_WEB_SPEC = "웹기술서"
DOC_EVIDENCE = "근거자료"  # 특허등록증·기능성 입증자료·인증서 등 표시 근거자료
DOC_UNKNOWN = "기타"

# 제조사(영업자) 인허가 서류 — 제품이 아니라 업체 단위라 유닛 전체에서 공유해 1단계 대조에 쓴다.
PERMIT_DOC_TYPES = (DOC_LICENSE, DOC_BIZ_REG, DOC_FACTORY_REG)
...
- 품목제조보고서(별지 제43호서식)·변경보고서(별지 제45호서식)는 정부 표준서식이라 칸이 고정이다.
  아래 표준 칸을 그대로 읽는다(보고서/변경보고서에 있을 때):
    · ★'보고인(신고인)' 칸과 '영업소' 칸을 반드시 구분하라. 영업자/제조원 정보는 '영업소' 칸에서만 읽는다:
      - business_name = '영업소'의 '명칭(상호)' (보고인 '성명'이 아님. 예: 영업소 '농업회사법인(주)우리식품').
      - license_no = '영업소'의 '영업등록번호' (보고인 '생년월일(법인등록번호)'가 아님 — 생년월일/주민·법인등록번호를 영업등록번호로 쓰지 말 것).
      - address = '영업소'의 '소재지' (보고인 '주소'가 아님. 보고인 주소는 개인 주소일 수 있다).
      - representative = 대표자/보고인 '성명'.
    · ★product_name = '제품명' 칸에 적힌 값만. '요청하는 품목제조보고번호'·'식품의 유형' 등은 칸 라벨이므로
      제품명으로 쓰지 말 것(라벨 문구를 제품명으로 추출하는 오류 금지).
    · manufacture_report_no = '요청하는 품목제조보고번호' 또는 '품목제조보고번호' 칸의 숫자값(라벨 텍스트가 아니라 숫자).
...
- 제조사(영업자) 인허가 서류 분류·추출(제품이 아니라 업체 서류):
    · '영업등록증'/'영업허가증'/'영업신고증' → doc_type='영업등록증'. business_name=영업소 명칭(상호),
      license_no=영업등록(허가)번호, address=소재지, representative=대표자, food_type=영업의 종류(예:
      식품제조·가공업, 수입식품등 수입·판매업).
    · '사업자등록증' → doc_type='사업자등록증'. business_name=상호(법인명), representative=대표자,
      license_no=사업자등록번호(또는 법인등록번호), address=사업장(본점) 소재지.
    · '공장등록증명서' → doc_type='공장등록증명서'. business_name=회사명, representative=대표자,
      address=공장 소재지, food_type=공장 업종.
  주의: 사업자등록증 주소는 '본점/사업장', 공장등록증 주소는 '공장(제조소)'이라 서로 다를 수 있다.
...
def render_file(
    ...
    if kind and kind.extension == "pdf":
        ...
        except Exception:  # noqa: BLE001 - 손상/암호화 PDF 등 로드 실패
            return []
        try:
            doc.init_forms()
        ...
        for page_index in page_indices:
            try:  # 한 페이지 렌더 실패('Failed to load page' 등)가 전체 검토를 막지 않도록 건너뜀
                page = doc[page_index]
