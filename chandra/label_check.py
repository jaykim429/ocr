...
- ★표시 근거자료: 라벨에 '특허 제○○호'/'특허 1017597790000호' 같은 특허번호, 또는 기능성·효능
  표현('뼈 건강', '○○에 도움', 기능성표시 등), 인증마크(HACCP·유기농·전통식품 등)가 있으면,
  그 표현·번호를 근거자료(특허등록증·기능성 입증자료·인증서)로 뒷받침해야 한다. 해당 표기가 보이면
  items 에 name='표시 근거자료 확인'(verdict='검토필요')로 '특허/기능성/인증 표현 발견 — 근거자료
  제출·일치 확인 필요(특허번호 ○○)'를 적는다. 표기가 없으면 추가하지 않는다.
