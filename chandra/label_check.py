...
def review_label_disclosures(
    ...
    **gemma_opts: Any,
    ...
    addr_check = (
        ...
        "차이는 동일로 보되, 읍/면/동·리·도로명·번지가 다르면(예: '내수읍'↔'수내읍') '소재지 표시 "
        "확인 필요'로 보고하세요. 숫자(번지)가 이미지상 흐릿하면 'OCR 오인식 가능'을, 명칭이 또렷이 "
        "다르면 '표시 오기 가능'을 reason 에 명시하세요. 단, OCR 텍스트가 아니라 이미지를 근거로 판단."
