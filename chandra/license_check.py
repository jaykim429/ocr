...
class LicenseCheckInput:
    ...
    # 인허가서류(보고서/등록증) 원본 OCR 텍스트 — 고유명사(상호·주소) 권위 기준(Gemma 오독 보정)
    report_ocr: str | None = None
...
- 영업등록번호가 DB와 일치(exists_in_db=true, db_match_basis=license_no)하면 영업자 존재가
  확정된 것이다. ★이때 DB 의 업소명(BSSH_NM)이 '권위 있는 정식 상호'다. 추출된 인허가서류·표시사항
  상호가 DB 업소명의 OCR/추출 오인식 변이(글자 1~2자 오인식 예 '린→런', 일부 음절 누락 예 '그린피시팜
  →그린피시')로 보이면 '동일 영업자'로 보고 name_match=true 로 두며 '상호 불일치'를 사유/검토 항목으로
  삼지 말 것. (license_no 가 같으면 같은 업체다 — DB 상호를 기준으로 채택한다.)
  명백히 다른 회사명일 때만 검토필요로 둔다. 그 외 다음도 '부적합' 아닌 '검토필요'로:
...
- ★'주소_정밀대조'가 null 이 아니면(표시사항 주소가 인허가/DB 주소와 전치(수내↔내수)·읍면동 명칭·
  번지 차이가 있음 — 1글자 OCR은 이미 제외됨) 주소를 '일치'로 단정하지 말고 address_match=false 로,
  그 차이를 reason 에 적고 overall_verdict 는 '검토필요'로 둔다(주소 표시 오기 가능성).
- ★'인허가서류_OCR원문(권위)'이 제공되면, 영업자명·주소 등 한글 고유명사의 정확한 표기는 이
  OCR 원문을 권위 기준으로 삼는다. 추출된 영업자명/주소(인허가서류 필드)가 OCR 원문과 다르면
  OCR 원문 표기를 신뢰하고 그 차이를 '불일치'로 판정하지 말 것(스캔 작은 글씨 추출 오독 가능성).
...
def check_license(
    ...
    **gemma_opts: Any,
    ...
    from chandra.address import label_address_discrepancy, verify_addresses
    ...
    # 정밀 대조: 표시사항 주소가 인허가/DB 주소와 '전치(수내↔내수)·명칭/번지' 차이가 있는지.
    # 1글자 OCR(철곡↔칠곡)은 보류, 2글자+ 차이만 잡아 느슨한 토큰매칭의 false-positive를 보완.
    addr_label_discrepancy = (
        label_address_discrepancy(data.label_address, data.address)
        or label_address_discrepancy(data.label_address, db_addr)
    )
    ...
    evidence = {
        ...
        "주소_정밀대조": addr_label_discrepancy,  # 표시사항 주소의 전치·명칭·번지 차이(있으면 검토필요)
        "인허가서류_OCR원문(권위)": (data.report_ocr or "")[:3000] or None,
        "api_error": api_error,
    }
